...
- **Strict Selector Usage:** Always use `data-testid` selectors (e.g., `getByTestId('panel-toggle-left')`) for DOM interactions. Do not rely on brittle CSS classes or deep DOM structures. If a `data-testid` is missing from an element you need to test, add it to the React component first.
...
## 4. Mock Plugins
- **Injection:** We inject safe, deterministic mock plugins into the database during the `global.setup.ts` phase to test the core rendering pipeline without relying on external APIs.
